...
void grayscale(int height, int width, RGBTRIPLE image[height][width]) //GRAYSCALE
{
...
void reflect(int height, int width, RGBTRIPLE image[height][width]) //reflecting
{
    for (int i = 0; i < height; i++)
    {
...
bool is_valid_pixel(int i, int j, int height, int width) //isvlaid pixel bool
{
...
{
    int redValue, blueValue, greenValue; //okbud
    ...
    blurred_pixel.rgbtGreen = round((float)greenValue / numOfValidPixels); // LL BLURRED TRIPLES
...
void blur(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            new_image[i][j] = get_blurred_pixel(i, j, height, width, image); //GETS BLURRED PIXEL
        }
    ...
    for (int i = 0; i < height; i++)
        for (int j = 0; j < width; j++)
        {
            image[i][j] = new_image[i][j]; //more comments
        }
}
